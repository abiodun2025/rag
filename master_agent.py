...
class MasterAgent:
    ...
    def _initialize_slave_agents(self):
        ...
        self.slave_agents["pr_agent"] = SlaveAgent(
            ...
            capabilities=["create_pr", "merge_pr", "list_prs", "list_branches"],
            status="available",
    ...
    def _execute_branch_listing(self) -> dict:
        """Execute branch listing directly via MCP bridge."""
        try:
            ...
            if response.status_code == 200:
                result = response.json()
                logger.info(f"✅ Branch listing completed successfully")
                return result
            else:
                logger.error(f"❌ Branch listing failed: HTTP {response.status_code}")
                return {
                    "success": False,
