#!/usr/bin/env python3
"""
Comprehensive testing script for all branches
...
class ComprehensiveBranchTester:
    ...
    async def check_branch_commits(self, branch_name: str) -> bool:
    ...
    async def run_pytest_tests(self, branch_name: str) -> bool:
    ...
    async def run_integration_tests(self, branch_name: str) -> bool:
    ...
    async def run_branch_specific_tests(self, branch_name: str) -> bool:
    ...
    async def run_comprehensive_testing(self):
        ...
        for branch in branches:
            ...
            has_commits = await self.check_branch_commits(branch_name)
            if not has_commits:
                self.print_result("Branch Commits", False, "No new commits", branch_name)
                continue
            
            # Checkout branch
            if not await self.checkout_branch(branch_name):
                continue
            
            # Run tests
            await self.run_pytest_tests(branch_name)
            await self.run_integration_tests(branch_name)
            await self.run_branch_specific_tests(branch_name)
            await self.test_email_body_issue(branch_name)
        
        # Test main branch last
        print(f"\n🌿 Testing Main Branch")
        await self.checkout_branch("main")
        await self.run_pytest_tests("main")
        await self.run_integration_tests("main")
        await self.run_branch_specific_tests("main")
        await self.test_email_body_issue("main")
