...
The system now features a **provider-independent schema architecture** that keeps the original database schema as the contract and creates separate provider-specific schemas for LLM requirements. This design:

- **Fixes Cohere API compatibility issues** by using provider-specific schemas
- **Future-proofs the system** for new LLM providers
- **Eliminates database schema changes** when adding new models
- **Provides clean separation** between data storage and provider requirements

### Key Changes:
- New `schemas.py` module with original database schemas and `ProviderSchema` for LLM compatibility
- Provider-specific schema conversion for Cohere, OpenAI, and other providers
- Original database schema (`sql/schema.sql`) remains unchanged as the contract
- Schema mapping layer handles conversion between database and provider formats
...
Execute the SQL in `sql/schema.sql` to create all necessary tables, indexes, and functions. This is the original database schema that serves as the contract.
...
# Voyage AI
EMBEDDING_PROVIDER=openai  # Uses OpenAI-compatible client
EMBEDDING_BASE_URL=https://api.voyageai.com/v1
EMBEDDING_API_KEY=your-voyage-api-key
EMBEDDING_MODEL=voyage-large-2
```

For Cohere (both LLM and embeddings):
```bash
# LLM Configuration
LLM_PROVIDER=cohere
LLM_API_KEY=your-cohere-api-key
LLM_CHOICE=command-r-plus
...
# Note: 
# - Don't set LLM_BASE_URL or EMBEDDING_BASE_URL for Cohere
# - The system automatically sets CO_API_KEY=LLM_API_KEY for pydantic_ai compatibility
# - Streaming responses use non-streaming mode for Cohere (full response sent at once)
```
