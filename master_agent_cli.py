...
def print_help():
    ...
    print("\n📋 Workflow Types:")
    print("  create_pr                       - Create a pull request")
    print("  pr_with_review                  - Create PR + review")
    print("  full_development_cycle          - Complete development workflow")
    print("  code_review                     - Review existing PR")
    print("\n📋 Examples:")
    print("  create pr_with_review 'My Feature' feature-branch")
...
Created at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}""",
            ...
            # For reviewing existing PRs
            try:
                pr_number = int(branch)  # Use branch parameter as PR number
                parameters = {
                    "pr_number": pr_number
                }
            except ValueError:
                print("❌ For code_review, provide a PR number instead of branch name")
                return
        else:
            print(f"❌ Unknown workflow type: {workflow_type}")
            return
        
        workflow_id = master.create_workflow(workflow_type, parameters, priority=1)
